...
| `list-google-requests` | `{ limit? }` | `{ requests: [{ received_at, intent, body }], llmResponses: [{ received_at, iter, costUsd, response }] }` — **superuser only**. Last N (default 100, max 100) raw Google fulfillment bodies *and* raw OpenRouter responses for this account, each newest-first. Sort/merge by `received_at` client-side if you want a single timeline. Errors for non-superusers. |
